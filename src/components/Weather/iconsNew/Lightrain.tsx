import * as React from "react";
import type { SVGProps } from "react";
const SvgLightrain = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    viewBox="0 0 100 100"
    width="1em"
    height="1em"
    {...props}
  >
    <symbol id="lightrain_svg__b">
      <path
        fill="#0062bf"
        d="M2.5 13A2.5 2.5 0 0 1 .21 9.51l3.55-8a2.5 2.5 0 0 1 4.57 2l-3.55 8A2.5 2.5 0 0 1 2.5 13Z"
      />
    </symbol>
    <symbol id="lightrain_svg__a">
      <path d="M55.7 5a23.94 23.94 0 0 0-21.33 13.05 9.9 9.9 0 0 0-12.78 5.56 15 15 0 0 0-1.71-.1A14.81 14.81 0 0 0 9.2 28 14.63 14.63 0 0 0 5 38.17v.21a14.83 14.83 0 0 0 14.88 14.68h55.71a14.3 14.3 0 0 0 3.67-28.14A23.93 23.93 0 0 0 55.7 5Z" />
      <image
        xlinkHref="data:image/png;base64,"
        width={85}
        height={43}
        x={5}
        y={14}
      />
    </symbol>
    <symbol id="lightrain_svg__c">
      <use
        xlinkHref="#lightrain_svg__a"
        width={100}
        height={100}
        fill="#ccc"
        transform="translate(3 18)"
      />
      <use
        xlinkHref="#lightrain_svg__b"
        width={100}
        height={100}
        transform="translate(32 87)"
      />
      <use
        xlinkHref="#lightrain_svg__b"
        width={100}
        height={100}
        transform="translate(56 78)"
      />
    </symbol>
    <use xlinkHref="#lightrain_svg__c" width={100} height={100} />
  </svg>
);
export default SvgLightrain;
